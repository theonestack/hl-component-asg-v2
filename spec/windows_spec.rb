...
describe 'compiled component' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/windows.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/windows/asg-v2.compiled.yaml") }
  ...
  context 'Resource LaunchTemplate' do
    ...
    it 'has windows userdata' do
      expect(userdata).to include("<powershell>")
    end
