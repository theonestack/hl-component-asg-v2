...
describe 'compiled component asg-v2' do
  ...
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/asg_scaling_simple/asg-v2.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "ScaleDownAlarm" do
      ...
      it "to have property Threshold" do
          expect(resource["Properties"]["Threshold"]).to eq("15")
      end
    ...
    context "ScaleUpPolicy" do
      ...
      it "to have property Cooldown" do
          expect(resource["Properties"]["Cooldown"]).to eq("60")
      end
      
      it "to have property ScalingAdjustment" do
          expect(resource["Properties"]["ScalingAdjustment"]).to eq(2)
      end
