...
describe 'compiled component asg-v2' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/iam_policies.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/iam_policies/asg-v2.compiled.yaml") }
