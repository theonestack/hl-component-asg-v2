...
describe 'compiled component asg-v2' do
  ...
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/asg_scaling_tracking/asg-v2.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "AverageCPUTracking" do
      ...
      it "to have property PolicyType" do
          expect(resource["Properties"]["PolicyType"]).to eq("TargetTrackingScaling")
      end
      
      it "to have property TargetTrackingConfiguration" do
          expect(resource["Properties"]["TargetTrackingConfiguration"]).to eq({"PredefinedMetricSpecification"=>{"PredefinedMetricType"=>"ASGAverageCPUUtilization"}, "TargetValue"=>60.0})
      end
      
      it "to have property EstimatedInstanceWarmup" do
          expect(resource["Properties"]["EstimatedInstanceWarmup"]).to eq(60)
      end
      
    end
    
    context "RequestCountTracking" do
      ...
      it "to have property TargetTrackingConfiguration" do
          expect(resource["Properties"]["TargetTrackingConfiguration"]).to eq({"PredefinedMetricSpecification"=>{"PredefinedMetricType"=>"ALBRequestCountPerTarget", "ResourceLabel"=>{"Fn::Sub"=>"${LoadBalancer}/${TargetGroup}"}}, "TargetValue"=>{"Ref"=>"ALBRequestCountTargetValue"}})
      end
